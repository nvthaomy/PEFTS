...
# Molecules
number_species = 4
charges = np.array([-1,1,-1,0], dtype = float)

number_molecules = 4
chargedPairs = [[0,1],[1,2]]       # only include pairs
PAADOP = 150
PAHDOP = 150
struc = [[0]*PAADOP,[1],[2],[3]]
molCharges = np.array([-1,1,-1,0], dtype = float) # charge per segment length of each molecule type

# Forcefield
u0 =[
[__B11__,__B16__,__B17__,__B15__],
[__B16__,__B66__,__B67__,__B56__],
[__B17__,__B67__,__B77__,__B57__],
[__B15__,__B56__,__B57__,__B55__]]

abead = [__a1__,__a6__,__a7__,__a5__]
lB = __lB__
b = [__b1__,__b6__,__b7__,__b5__]
...
data.write('# CPAA CNa CCl CHOH Ctot fI fII CI1 CII1 CI2 CII2 CI3 CII3 CI4 CII4 dP dmuPAANa dmuNaCl dmuW PI PII relDeltaG calculated_P_bulk fracErr C3\n')
data.flush()
...
for i in range(nC):
    try:
        os.mkdir('CNa{}'.format(round(C3,5)))
    except:
        pass
    os.chdir('CNa{}'.format(round(C3,5)))
    print('==CNa {}=='.format(round(C3,5)))
    log.write('==CNa {}=='.format(round(C3,5)))
    log.flush()
    ...
    if i == 0:
        Cs = np.array([C1_0,C3_0,C4_0,C5_0])
    else:
        Cs = np.array([C1,C3,C4,C5])
    xs = Cs/sum(Cs)
    ...
    while fracErr > GibbsTolerance:
        ...
        # NaCl (salt species)
        CNa_sI = CI[1] - CI[0] * np.abs(molCharges[0])
        CCl_sI = CI[2] 
        #CNa_sI = CI[1]
        #CCl_sI = CI[2] - CI[0] * np.abs(molCharges[0])
        CNa_sII = CII[1] - CII[0] * np.abs(molCharges[0])
        CCl_sII = CII[2] 
        #CNa_sII = CII[1]
        #CCl_sII = CII[2] - CII[0] * np.abs(molCharges[0])
        ...
        CSaltI = CNa_sI
        CSaltII = CNa_sII
        CSaltI = CSaltI - DtCpair[1] * dmuEff[1]
        # PAA or PAH
        ...
        CI[0] = CI[0] + dt
        # HOH
        CI[3] = CI[3] - Dt[-1] * np.min([CI[3], CII[3]])/5. * dmuEff[2]
        
        # Get CNa and CCl
        CI[1] = CSaltI + CI[0] * np.abs(molCharges[0])
        CI[2] = CSaltI
        #CI[1] = CSaltI
        ...
        if CI[1] < 0: # set to minimum concentration for electroneutrality
            CI_old = CI[1]
            CI[1] = 1e-5
            CI[2] += (CI[1]-CI_old)
        elif CI[2] < 0:
            ...
            CI[2] = 1e-5
            CI[1] += (CI[2]-CI_old) 
        if CI[1] > Cs[1]/fI: # if na+ of cl- are over the max range, reduce the concentration of both ions 
            CI_old = CI[1]
            CI[1] = 0.99 * Cs[1]/fI
            CI[2] = CI[2] - (CI_old-CI[1])
        elif CI[2] > Cs[2]/fI:
            CI_old = CI[2]
            CI[2] = 0.99 * Cs[2]/fI
            CI[1] = CI[1] - (CI_old-CI[2])
...
#            DtCtot = DtCtot0 * 1.5
    ...
    if fI < 0.3 or fI > 0.7:
        ...
        if not 'nan' in s and not 'inf' in s:
            ...
            shiftBulk = False
            Ctot_tmp = np.sum(np.array([C1_0,C3_0,C4_0,C5_0]))
