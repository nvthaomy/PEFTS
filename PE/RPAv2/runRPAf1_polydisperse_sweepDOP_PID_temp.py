...
PAANns = np.arange(50,160,1)
PAHNns = np.arange(50,160,1)
PAADs = np.ones(len(PAANns))
PAHDs = np.ones(len(PAANns))
nC = len(PAANns)
...
data = open(dataFile,'w')
data.write('# CPAAs CPAHs CNa CCl CHOH Ctot fI fII [CIPAA CIIPAA]s [CIPAH CIIPAH]s CINa CIINa CICl CIICl CIHOH CIIHOH  dP dmuPAANa_s dmuPAHCl_s dmuNaCl dmuW PI PII relDeltaG calculated_P_bulk fracErr Csalt NnPAA NnPAH DPAA DPAH\n')
data.flush()
...
for i in range(nC):
    #number average DOP:
    Nns = [PAANns[i],PAHNns[i],1,1,1]
    # dispersity index of each molecule type:
    Ds = np.array([PAADs[i], PAHDs[i], 1., 1., 1.]) 
    ...
    if i == 0:
        ...
        Cs_Init_5species = [np.sum(C0s[molId['PAA']]),np.sum(C0s[molId['PAH']]),np.sum(C0s[molId['Na']]),np.sum(C0s[molId['Cl']]),np.sum(C0s[molId['HOH']])] # combine conc of different chain length and save for future use
        CIs_Init_5species = [np.sum(CI0s[molId['PAA']]),np.sum(CI0s[molId['PAH']]),np.sum(CI0s[molId['Na']]),np.sum(CI0s[molId['Cl']]),np.sum(CI0s[molId['HOH']])] 
        fI_Init = fI0
    else:
        if not 'nan' in s and not 'inf' in s: 
            struc, rep, C0s , chargedPairs, molCharges, molId = ParsePoly(Ds,Nns,Cs_converged,struc0,molCharges0,chargedPairs0)
            _, _, CI0s , _, _, _ = ParsePoly(Ds,Nns,CIs_converged,struc0,molCharges0,chargedPairs0)
        else:
            struc, rep, C0s , chargedPairs, molCharges, molId = ParsePoly(Ds,Nns,Cs_Init_5species,struc0,molCharges0,chargedPairs0)
            _, _, CI0s , _, _, _ = ParsePoly(Ds,Nns,CIs_Init_5species,struc0,molCharges0,chargedPairs0)
    ...
    try:
        os.mkdir('PAA{}-{}_PAH{}-{}'.format(PAANns[i],PAADs[i],PAHNns[i],PAHDs[i]))
    except:
        pass
    os.chdir('PAA{}-{}_PAH{}-{}'.format(PAANns[i],PAADs[i],PAHNns[i],PAHDs[i]))
    print('==PAA{}-{}_PAH{}-{}=='.format(PAANns[i],PAADs[i],PAHNns[i],PAHDs[i]))
    log.write('==PAA{}-{}_PAH{}-{}=='.format(PAANns[i],PAADs[i],PAHNns[i],PAHDs[i]))
    log.flush()
    ...
    #            DtCtot = DtCtot0 * 1.5
        ...
        if not NAN:
            fI_converged = fI
            Cs_converged = [np.sum(Cs[molId['PAA']]),np.sum(Cs[molId['PAH']]),np.sum(Cs[molId['Na']]),np.sum(Cs[molId['Cl']]),np.sum(Cs[molId['HOH']])]
            CIs_converged = [np.sum(CI[molId['PAA']]),np.sum(CI[molId['PAH']]),np.sum(CI[molId['Na']]),np.sum(CI[molId['Cl']]),np.sum(CI[molId['HOH']])]
            data.write(s)
            data.flush()
        else: #increase salt concentration
            print('increase salt concentration')
            log.write('increase salt concentration\n')
            # go back to the initial compositions
            if i != 0:
                fI = fI_converged
                Cs_converged[2] += dC3
                Cs_converged[3] += dC3
                _, _, Cs , _, _, _  = ParsePoly(Ds,Nns,Cs_converged,struc0,molCharges0,chargedPairs0)
                _, _, CI0s , _, _, _ = ParsePoly(Ds,Nns,CIs_converged,struc0,molCharges0,chargedPairs0)
            else:
                fI = fI_Init
                Cs_converged = Cs_Init_5species.copy()
                Cs_converged[2] += dC3
                Cs_converged[3] += dC3
                _, _, Cs , _, _, _  = ParsePoly(Ds,Nns,Cs_converged,struc0,molCharges0,chargedPairs0)
                _, _, CI0s , _, _, _ = ParsePoly(Ds,Nns,CIs_Init_5species,struc0,molCharges0,chargedPairs0)
